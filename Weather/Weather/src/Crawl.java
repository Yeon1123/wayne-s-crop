...
public class Crawl{
	...
	public String getDiv(String url) throws IOException { // atag인 것들 가져오기.
		Document doc = Jsoup.connect(url).get();
		Elements elemA = doc.select("div.row.clear_both"); // a tag인  td들 중 클래스가 title인 것만 받아오기.
        String strA = elemA.outerHtml(); // jsoup의 메소드를 이용해서 html 가져오기.
        return strA;
	}
